; Bootloader que carrega kernel e o GDT 
...
start:
	...
	jmp 0x08:0x8200	    ;chamado de "far jump" é definido por: <segment>:<address_offset> ;pula para o segmento de código
...
load_kernel:
	...
	;; IMPORTANTE: DEPENDENDO DO TAMANHO DO TEXTO DE CÓDIGO DEVEMOS ;;
	;;	AUMENTAR A QUANTIDADE DE SETORES A SEREM LIDOS!		;;
	;;		(instrução logo abaixo)				;;
	;15 setores	
	...
	;parametro do endereco, onde o kernel será carregado 
	;lembrando que a fórmula para o gerenciamento é feita por: 
	;BX_ADRESS = ADRESS_OFFSET dividido por 16 (valores em decimais) ou ADRESS_OFFSET dividido por 0x10 (valores em hexadecimais)
	;para o exemplo dado, como queremos carregar em 0x8200, dividiremos o valor por 0x10 (hexa), logo:
	;0x8200/0x10 = 0x820 (offset a ser movido para bx)
...
; FIXAÇÃO DE TAMANHO / ASSINATURA DO BOOTLOADER ;
