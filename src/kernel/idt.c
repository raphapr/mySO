...
//;-------------- ARQUIVOS EXTERNOS EM ASSEMBLY ----------------;

//idt_load.asm
//função de carregamento da IDT, passando o ponteiro no parâmetro
...
//inicia a idt, seta o limite e o tamanho
void idt_init(void)
{
	idt_ptr.limit = (sizeof(idt_entry_t) * NUM_INTERRUPTIONS) - 1; 	//((tamanho da Idt (struct)) * NUM_INTERRUPTIONS) - 1
	idt_ptr.base  = (unsigned int) &idt; //ponteiro para a tabela idt

	
	memset(&idt, 0, sizeof(idt_entry_t)*256);

	//devemos remapear a tabela de IRQ para evitar inconsistência (interruption request)
	outb(0x20, 0x11);
	outb(0xA0, 0x11);
	outb(0x21, 0x20);
	outb(0xA1, 0x28);
	...
	//0x10 = codigo do selector de código
	...
	//chama o ponteiro e carrega a idt para utilização
